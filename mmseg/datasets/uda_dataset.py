...
from mmseg.utils import DataContainer as DC
...
class UDADataset(object):
    ...
    def __init__(self, **cfg):
        if isinstance(cfg, dict):
            source = DATASETS.build(cfg['source'])
            target = DATASETS.build(cfg['target'])
        else:
            raise TypeError(
                'elements in datasets sequence should be config or '
                f'`ConcatDataset` instance, but got {type(cfg)}')

        self.source = source
        self.target = target
        self.ignore_index = target.ignore_index
        self._metainfo = target._metainfo
        assert target.ignore_index == source.ignore_index
        assert target.METAINFO['classes'] == source.METAINFO['classes']
        assert target.METAINFO['palette'] == source.METAINFO['palette']
        ...
        if self.rcs_enabled:
            ...
            print_log(f'RCS Classes: {self.rcs_classes}', logger="current")
            print_log(f'RCS ClassProb: {self.rcs_classprob}', logger="current")
            ...
            self.file_to_idx = {}
            for i, data in enumerate(self.source.load_data_list()):
                file = osp.basename(data['seg_map_path'])
    ...
    def get_rare_class_sample(self):
        ...
        if self.rcs_min_crop_ratio > 0:
            for j in range(10):
                ...
                # print_log(f'{j}: {n_class}', logger="current")
