...
def get_mean_std(img_metas, dev):
    mean = [
        torch.as_tensor(img_metas['img_norm_cfg']['mean'][i], device=dev)
        for i in range(len(img_metas['img_norm_cfg']['mean']))
    ]
    mean = torch.stack(mean).view(-1, 3, 1, 1)
    std = [
        torch.as_tensor(img_metas['img_norm_cfg']['std'][i], device=dev)
        for i in range(len(img_metas['img_norm_cfg']['mean']))
    ]
