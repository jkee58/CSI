...
def prepare_debug_out(title, out, mean, std):
    ...
    if out.shape[0] == 3:
        ...
        out = dict(title=title, image=out)
    elif out.shape[0] > 3:
        out = torch.softmax(torch.from_numpy(out), dim=0).numpy()
        out = np.argmax(out, axis=0)
        out = dict(title=title, image=out, cmap='cityscapes')
    elif out.shape[0] == 1:
        if is_integer_array(out) and np.max(out) > 1:
            out = dict(title=title, image=out[0], cmap='cityscapes')
        elif np.min(out) >= 0 and np.max(out) <= 1:
            out = dict(title=title, image=out[0], cmap='viridis', vmin=0, vmax=1)
        else:
