...
class MaskingConsistencyModule(Module):
    ...
    def __call__(self,
        ...
        gt_semantic_seg = self._stack_batch_gt(src_data['data_samples'])
        ...
        # Use 1x source image and 1x target image for MIC
            ...
            assert src_data['inputs'].shape[0] == 2
            masked_img = torch.stack(
                [src_data['inputs'][0], trg_data['inputs'][0]])
        ...
        masked_data = deepcopy(src_data)
        # Apply masking to image
        masked_data['inputs'] = self.mask_gen.mask_image(masked_img)
        for data_sample, lbl in zip(masked_data['data_samples'],
                                    masked_lbl):  ## TO DO: REFACT
            data_sample.gt_sem_seg.data = lbl

        # Train on masked images
        masked_loss = model.loss(
            masked_data['inputs'],
            masked_data['data_samples'],
