...
class HRDAHead(BaseDecodeHead):
    ...
    def loss(self, inputs: Tuple[Tensor], batch_data_samples: SampleList, train_cfg: ConfigType, seg_weight=None, return_logits=False) -> dict:
    ...
    def predict(self, inputs, batch_img_metas, test_cfg):
        ...
        return self.predict_by_feat(fused_seg, batch_img_metas)

    def loss_by_feat(self, seg_logit, batch_data_samples, seg_weight=None):
        """Compute losses."""
        fused_seg, lr_seg, hr_seg = seg_logit
        seg_labels = self._stack_batch_gt(batch_data_samples)
        loss = super(HRDAHead, self).loss_by_feat(fused_seg, batch_data_samples, seg_weight)
        ...
        if self.lr_loss_weight > 0:
            loss.update(
                add_prefix(
                    super(HRDAHead, self).loss_by_feat(lr_seg, batch_data_samples, seg_weight), 'lr'))
        if self.hr_loss_weight > 0 and self.enable_hr_crop:
            cropped_batch_samples = [batch_data_sample.detach().clone() for batch_data_sample in batch_data_samples]
            cropped_seg_labels = crop(seg_labels, self.hr_crop_box)
            for cropped_batch_sample, cropped_seg_label in zip(cropped_batch_samples, cropped_seg_labels):
                cropped_batch_sample.gt_sem_seg = PixelData(data=cropped_seg_label)
            ...
            if self.debug:
                self.debug_output['Cropped GT'] = \
                    cropped_seg_labels.squeeze(1).detach().cpu().numpy()
            loss.update(
                add_prefix(
                    super(HRDAHead, self).loss_by_feat(hr_seg, cropped_batch_samples,
                                                 cropped_seg_weight), 'hr'))
            del cropped_batch_samples
        elif self.hr_loss_weight > 0:
            loss.update(
                add_prefix(
                    super(HRDAHead, self).loss_by_feat(hr_seg, batch_data_samples,
                                                 seg_weight), 'hr'))
        ...
        if self.debug:
            self.debug_output['GT'] = \
                seg_labels.squeeze(1).detach().cpu().numpy()
